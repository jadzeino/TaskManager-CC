...
export async function getUserById(req: Request, res: Response) {
    try {
        ...
        const { id } = req.params;
        const [userRows] = await pool.query<any>('SELECT id, name, email FROM users WHERE id = ?', [id]);
...
export async function loginUser(req: Request, res: Response) {
    try {
        ...
        return res.status(200).json({ message: 'Login successful', userId: user.id,name:user.name,email:user.email });
      } catch (error) {
...
export async function createUser(req: Request, res: Response) {
    ...
    try {
        ...
        const [existingUserRows] = await pool.query<any>('SELECT * FROM users WHERE email = ?', [email]);
...
export async function updateUser(req: Request, res: Response) {
  try {
    ...
    const [userRows] = await pool.query<any>('SELECT * FROM users WHERE id = ?', [id]);
    const user = userRows[0];
