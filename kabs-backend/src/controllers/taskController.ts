...
import { createDatabasePool, pool } from "../db/database";
import {
  createTaskHistory,
...
export async function updateTaskStatus(req: Request, res: Response) {
  try {
    ...
    if (
      !status ||
      !["ToDo", "InProgress", "Blocked", "InQA", "Done", "Deployed"].includes(
        status
      )
    ) {
    ...
    try {
      ...
      await pool.query(
        "UPDATE tasks SET status = ?, updatedBy = ?, updatedAt = NOW() WHERE id = ?",
...
export async function updateTaskTitleAndDescription(
  req: Request,
  res: Response
) {
  try {
    ...
    try {
      ...
      await pool.query(
        "UPDATE tasks SET title = ?, description = ?, updatedBy = ?, updatedAt = NOW() WHERE id = ?",
        [title, description, updatedBy, taskId]
      );
      ...
      return res
        .status(200)
        .json({ message: "Task title and description updated successfully" });
    } catch (error) {
      // Rollback the transaction in case of error
      await connection.rollback();
...
export async function assignTask(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      await pool.query(
        "UPDATE tasks SET assignedTo = ?, updatedBy = ?, updatedAt = NOW() WHERE id = ?",
        [targetUserId, updatedBy, taskId]
      );
...
export async function getTaskHistory(req: Request, res: Response) {
  try {
    ...
    const [historyRows] = await pool.query(
      "SELECT * FROM task_history WHERE taskId = ? ORDER BY changedAt DESC",
      [taskId]
    );
