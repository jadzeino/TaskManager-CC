...
describe('validateTaskStatusTransition', () => {
    it('should not throw an error for valid status transition', () => {
      expect(() => validateTaskStatusTransition('ToDo', 'InProgress')).not.toThrow();
      expect(() => validateTaskStatusTransition('InProgress', 'Blocked')).not.toThrow();
      expect(() => validateTaskStatusTransition('InProgress', 'InQA')).not.toThrow();
      ...
      expect(() => validateTaskStatusTransition('Done', 'Deployed')).not.toThrow();
    });
  
    it('should throw an error for invalid current status', () => {
      expect(() => validateTaskStatusTransition('InvalidStatus', 'InProgress')).toThrow(
        'Invalid current status: InvalidStatus'
      );
    });
