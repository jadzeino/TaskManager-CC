...
export async function createTaskHistory(
    ...
    try {
      ...
      const insertHistoryQuery = 'INSERT INTO task_history (taskId, field, oldValue, newValue, changedBy, changedAt, message) VALUES (?, ?, ?, ?, ?, ?, ?)';
      await pool.query(insertHistoryQuery, [taskId, field, oldValue, newValue, changedBy, changedAt, message]);
    } catch (error) {
      // Handle error
...
export function validateTaskStatusTransition(currentStatus: string, newStatus: string) {
